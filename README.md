...
> **Atenção:** este monitor roda em **self-hosted runner** (VPS própria), não nos servidores do GitHub. O portal da ALEGO usa o sistema Agape, que bloqueia IPs de datacenter como os da AWS/GitHub.
...
Todos os outros tipos são ignorados — não geram email, mas são marcados como vistos para não reaparecer em runs futuros.
...
2. O script acessa `alegodigital.al.go.leg.br/spl/consulta-producao.aspx`
3. Navega pelas páginas via postback ASP.NET UpdatePanel (sistema Agape — mesmo da ALES-ES, CMV-ES e CMC-MT)
...
3. Busque **"Senhas de app"** → Criar → nome: `monitor-alego-go`
...
2. Nome: `monitor-proposicoes-alego-go` | Visibilidade: **Private**
...
mkdir actions-runner-alego-go && cd actions-runner-alego-go
...
./config.sh --url https://github.com/SEU_USUARIO/monitor-proposicoes-alego-go --token TOKEN_GERADO
...
1. **Actions → Monitor Proposições ALEGO-GO → Run workflow → Run workflow**
...
```bash
curl -I https://alegodigital.al.go.leg.br/spl/consulta-producao.aspx
```
Se retornar 200, o problema é outro. Se recusar conexão, o IP da VPS foi bloqueado.

**`0 proposições novas` mas deveria ter**
→ Verifique se o nome do tipo aparece exatamente como no array `TIPOS_MONITORADOS` dentro do `monitor.js`. O filtro é case-insensitive mas precisa de correspondência textual. Se necessário, adicione a variação e faça commit.
